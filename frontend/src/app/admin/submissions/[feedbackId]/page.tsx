      ...
      >
        <div className="p-4">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
            ...
            <span>
              {format(new Date(submission.submission.submittedAt), "PPP")}
            </span>
          </div>
    ...
    <HoverCard>
      <HoverCardTrigger>
        ...
        >
          ...
          <div
            className="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-gradient-to-br opacity-10 rounded-full"
            style={{
              background: `conic-gradient(from 90deg at 40% 40%, ${colors[color]})`,
            }}
          />
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {metrics.map((metric, index) => (
          <div
            key={index}
            className={cn("p-4 rounded-xl", colors[color].light)}
          >
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              {metric.change && (
                <Badge className={cn("bg-white", colors[color].text)}>
                  {metric.change}
                </Badge>
              )}
            </div>
        ...
        {recommendations.map((rec, index) => (
          ...
          >
            ...
            <div>
              ...
              <Badge className={cn("mt-2 bg-white", colors[color].text)}>
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
      ...
      <p className="text-gray-500 max-w-md mb-6">
        This feedback form hasn&apos;t received any submissions yet. Please wait
        for participants to submit their responses.
      </p>
      <Button
        variant="outline"
...
export default function FeedbackSubmissionPage({
  ...
  const [selectedSubmissionId, setSelectedSubmissionId] = useState<
    number | null
  >(null);
  const [satisfactionAnalysis, setSatisfactionAnalysis] =
    useState<SatisfactionAnalysis | null>(null);
        ...
        if (data.submissions?.length > 0) {
          ...
          if (validSubmissions.length > 0) {
            ...
            setSatisfactionAnalysis(
              data.satisfaction as unknown as SatisfactionAnalysis,
            );
  ...
  const filteredSubmissions = submissions.filter(
    (submission) =>
      ...
      (submission.submission.submittedBy?.toString() || "").includes(
        searchQuery,
      ),
  );
  ...
  const analyzedCount = submissions.filter(
        ...
        <div className="relative bg-white/80 backdrop-blur-sm p-6 rounded-xl shadow-lg border border-white/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            ...
            <StatCard
              ...
              value={
                isRefreshing ? "..." : `${analyzedCount}/${submissions.length}`
              }
              icon={ChartBar}
              change={
                isRefreshing
                  ? null
                  : `${((analyzedCount / submissions.length) * 100).toFixed(1)}% analyzed`
              }
              color="blue"
            />
            <StatCard
              ...
              value={
                isRefreshing
                  ? "..."
                  : satisfactionAnalysis
                    ? `${Math.min(Math.max(satisfactionAnalysis.satisfactionOverview.satisfactionRate, 0), 100).toFixed(1)}%`
                    : "N/A"
              }
              icon={Smile}
            ...
            <StatCard
              ...
              value={
                isRefreshing
                  ? "..."
                  : aiInsights
                    ? `${aiInsights.insights.performanceInsights.aiConfidence.toFixed(1)}%`
                    : "N/A"
              }
              icon={Cpu}
              color="yellow"
                ...
                {aiInsights && (
                  <div className="lg:col-span-2">
                    <InsightCard
                      ...
                      recommendations={
                        aiInsights.insights.performanceInsights.recommendations
                      }
                      icon={Target}
                      color="violet"
                    />
                  ...
                  <Card className="p-6 bg-white shadow-lg border-0">
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-gray-500 flex flex-row gap-2 items-center">
                              <ChartColumnBig className="h-5 w-5 text-red-500" />
                              Analysis Progress
                            </span>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-gray-500 flex flex-row gap-2 items-center">
                              <Meh className="h-5 w-5 text-green-500" />
                              Satisfaction Rate
                            </span>
                    ...
                    {selectedSubmission ? (
                      <div className="space-y-6">
                        <SubmissionDetails
                          ...
                          analysis={
                            selectedSubmission.analysis as unknown as ImportedAnalysis
                          }
                        />
                      </div>
                    ) : (
