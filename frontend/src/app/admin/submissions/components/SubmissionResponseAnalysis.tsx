...
export function SubmissionResponseAnalysis({
  submissionData,
}: SubmissionResponseAnalysisProps) {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-emerald-50 rounded-lg">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(
                  new Set(submission.questionDetails.map((q) => q.category)),
                ).map((category, index) => (
                  <Badge
                    key={index}
                    className={cn("px-3 py-1", getCategoryColor(category))}
                  >
                    {category
                      .split("_")
                      .map(
