...
import type {
  ...
  CreateFeedbackDto,
  FeedbackFilters,
} from "@/app/admin/feedbacks/models/types";
...
export async function getFeedbacks(filters: Partial<FeedbackFilters> = {}) {
  try {
    const response = await feedbackApi.get("/api/v1/admin/feedbacks/get-all", {
      params: filters,
    });
...
export async function deleteFeedback(id: number) {
  try {
    const response = await feedbackApi.delete(
      `/api/v1/admin/feedbacks/delete/${id}`,
    );
    return response.data;
  } catch (error) {
...
export async function toggleFeedbackStatus(id: number, active: boolean) {
  try {
    const response = await feedbackApi.patch(
