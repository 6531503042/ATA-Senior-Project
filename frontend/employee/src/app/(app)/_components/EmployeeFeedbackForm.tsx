...
import type { Question, AnswerOption } from '@/app/types/question';
...
export default function EmployeeFeedbackForm({ projectName, questions, onSubmit }: Props) {
  ...
  const handleSubmit = () => {
    const normalized = questions.map((q) => {
      ...
      if (q.type === 'rating') value = typeof value === 'number' ? value : Number(value);
        ...
        {questions.length === 0 ? (
          ...
          questions.map((q) => {
            switch (q.type) {
              ...
              case 'single_choice': {
                const selected = typeof answers[q.id] === 'string' && answers[q.id]
                  ? new Set([answers[q.id] as string])
                  : new Set<string>();
                return (
                  <div key={q.id} className="space-y-2 rounded-xl border border-default-200 p-3">
                    <div className="text-sm font-medium text-default-700">
